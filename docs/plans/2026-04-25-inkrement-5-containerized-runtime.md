...
After Inkrement 4 finished the catalog/capability model, the user clarified that the runtime should be **fully containerized**: not just specifyr itself (already in a Docker image), but every Hermes agent process. This shifts the runtime architecture meaningfully and replaces the previously-discussed Bash-wrapper approach to capability enforcement.
...
### In Scope
- A new `hermes-docker.js` runner in specifyr that spawns each agent in its own Docker container.
...
- Networking topology so each agent's `company-ops` MCP server can reach specifyr's HTTP API by service name.
...
│                                               ▼               │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ specifyr (Nuxt + orchestrator + CompanyRuntime)        │  │
│  │  - reads agent specs                                    │  │
...
| `payment:execute_unrestricted` (sensitive) | container additionally has the configured payment provider's binary/credentials, **and** every operation triggers specifyr ApprovalService gate via `company-ops` |
...
- Network: attaches to a Compose-defined network where specifyr is reachable as `specifyr:3000`
- `capability-gate`-aware: refuses `--privileged`, refuses to mount `/`

### 5.3 — Compose updates

`docker-compose.yml` gets a second service for the agent network:
- `specifyr` service (existing) needs `/var/run/docker.sock` mounted with read-only-by-default
- New named network `companies` that specifyr and agent containers join
...
5. **Approval gate UX**: when sensitive capabilities trigger `requiresApproval`, the approval flows through specifyr's UI. The agent container blocks waiting on company-ops MCP response. Need a timeout / fallback.

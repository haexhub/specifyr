...
if docker ps --format '{{.Names}}' | grep -qx specifyr-postgres-dev; then
  ...
  echo "Postgres ist down — restliche DB-Checks ergeben keinen Sinn." >&2
  exit 1
fi
...
if [ "$DB_EXISTS" = "1" ]; then
  ...
  JOURNAL=$(grep -c '"tag"' \
    /home/haex/Projekte/specifyr/server/shared/database/migrations/meta/_journal.json \
    2>/dev/null || echo 0)
...
if docker ps --format '{{.Names}}' | grep -qx specifyr-dev \
  ...
  if [ -n "$KEY_SPEC" ] && [ "$KEY_SPEC" = "$KEY_PROXY" ]; then
    ...
    bad "Key unset in mind. einem Container.  Fix: SPECIFYR_SECRET_KEY=\$(openssl rand -hex 32) in .env, dann docker compose up -d --force-recreate"
  else

...
// AgentDetailDrawer — side drawer that slides in from the right when an
// agent is selected in the Runtime-View org chart. Shows:
//   - Agent role + reports_to + delivers_to (from spec)
//   - Capabilities + resources
//   - Currently dispatched task (if any) for this role
//   - Iteration chain: parent_task_id → parent's parent → ... back to root
//   - Recent events filtered by role
//
// Why custom instead of shadcn <Sheet>: the project doesn't ship a Sheet
// primitive yet (only Dialog, which is centered-modal). A drawer is two
// CSS rules + a transition; building it inline is faster than installing
// a new shadcn component and easier to style consistently with the rest.
//
// Selection state lives in the URL (?agent=<role>) — see runtime.vue.
// Closing emits 'close' which clears the URL param.
...
// Events filtered to this role. Newest-first (events array is already
...
// Currently in-flight task for this role (if any). Pending list is shared
// across all roles; pick the entry whose role matches.
...
// Iteration-chain reconstruction: walk parent_task_id backwards through the
// event log. We use task_path as the correlation key (one task per file path).
// The chain shows where this iteration came from.
//
// Limitation: we only see what's in the polled `events` window. If the chain
// extends further back than 50 events, we truncate at "older". Long chains
// will need 10b's SQLite for full lookup — not in scope here.
const iterationChain = computed<EventRow[]>(() => {
  if (!currentTask.value) return [];
  const chain: EventRow[] = [];
  let cursor: string | null = currentTask.value.parent_task_id;
  // Build a quick lookup from task_path → started-event so we can walk back.
  const byPath = new Map<string, EventRow>();
  for (const e of props.events) {
    if (e.type === "dispatch-started" && e.task_path) {
      // Keep the earliest start per path (events are newest-first; overwrite
      // would keep the latest, which is wrong for chain reconstruction).
      if (!byPath.has(e.task_path)) byPath.set(e.task_path, e);
    }
  }
  // Heuristic: the chain references task IDs (built by buildTaskId), but the
  // event index doesn't promote task_id as a column. Match by suffix in
  // task_path which contains the task_id.
...
// Close on Esc. Only when open; ignored otherwise.
      ...
      <div v-if="open" class="fixed inset-0 z-40 flex" @click.self="emit('close')">
        ...
        <aside class="flex h-screen w-[28rem] shrink-0 flex-col border-l border-border bg-background shadow-xl">
          ...
          <div class="flex-1 space-y-5 overflow-y-auto px-5 py-4 text-sm">
            <!-- Hierarchy -->
            <section v-if="agent">
              <p class="mb-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Hierarchie</p>
            ...
            <section v-if="agent?.capabilities?.length">
              <p class="mb-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Capabilities</p>
            ...
            <section v-if="agent?.resources">
              <p class="mb-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Resources</p>
              <div class="space-y-1 text-muted-foreground">
            ...
            <!-- Current Task -->
            <section>
              <p class="mb-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Aktueller Task</p>
              <div v-if="currentTask" class="rounded-md border border-blue-500/30 bg-blue-500/5 px-3 py-2 text-xs">
                ...
                <div class="mt-1 text-muted-foreground">seit {{ relativeTime(currentTask.at) }}</div>
              </div>
              <div v-else class="text-xs text-muted-foreground">Aktuell kein Task in Bearbeitung.</div>
            </section>
            ...
            <section v-if="iterationChain.length">
              <p class="mb-1.5 flex items-center gap-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
                ...
                Iteration-Kette (parent_task_id)
              </p>
...
<style scoped>
/* Drawer slide-in from the right. Uses Vue's <Transition> name conventions:
   .drawer-enter-from / .drawer-leave-to == hidden state. */
.drawer-enter-active,
.drawer-leave-active {

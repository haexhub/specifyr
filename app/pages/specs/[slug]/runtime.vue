...
//
// Three logically distinct panes that must NOT mix concerns
// (architecture_decisions.md §1):
...
//
// Polling is fine for v1; the polling intervals are deliberately conservative
// because the underlying source of truth is files-on-disk and SQLite, neither
// of which we want to hammer.

import { ChevronLeft, Activity, Network, History, Circle } from "lucide-vue-next";
...
const { data: companyStatus, refresh: refreshStatus } = await useFetch<CompanyStatus>(
  ...
  { default: () => ({ slug: slug.value, status: "idle" } as CompanyStatus), key: () => `cstatus-${slug.value}` }
);

const isRunning = computed(() => companyStatus.value?.status === "running");

// Events — refresh every 5s while running. Only fetch when running, otherwise 404.
...
// Pending dispatches — derived from event log: dispatch-started without
// a matching terminal event (completed/failed/error) on the same task_path.
...
// Org chart — group agents by reports_to to render a tree.
  ...
  <div class="min-h-screen p-6 lg:p-10">
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <header class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p class="text-xs uppercase tracking-[0.18em] text-muted-foreground">{{ slug }}</p>
          <h1 class="text-2xl font-semibold tracking-tight">Runtime-View</h1>
        </div>
        <div class="flex items-center gap-3">
          <NuxtLink :to="`/specs/${slug}`" class="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
            <ChevronLeft class="size-4" /> Speckit-View
          </NuxtLink>
          <Badge :variant="isRunning ? 'default' : 'secondary'">
            <Circle class="mr-1 size-2" :class="isRunning ? 'fill-green-500 text-green-500' : 'fill-muted-foreground text-muted-foreground'" />
      ...
      <Card v-if="!isRunning">
        <CardContent class="py-10 text-center text-sm text-muted-foreground">
          Die Company läuft gerade nicht. Starte sie über
          <NuxtLink :to="`/specs/${slug}`" class="text-primary hover:underline">Speckit-View → Run</NuxtLink>,
          dann erscheint hier Live-Status, Org-Chart und History.
        </CardContent>
      </Card>
      ...
      <template v-else>
        ...
        <Card>
          ...
          <CardContent class="space-y-2">
            ...
            <div v-for="p in pendingDispatches" :key="p.id" class="flex items-center justify-between rounded-md border bg-muted/30 px-3 py-2 text-sm">
              <div>
        ...
        <Card>
          ...
          <CardContent class="space-y-3">
            <div v-for="root in orgRoots" :key="root.role" class="space-y-2">
              ...
              <div v-if="childrenOf(root.role).length" class="ml-6 space-y-2 border-l border-muted pl-4">
                <div v-for="child in childrenOf(root.role)" :key="child.role" class="rounded-md border bg-muted/30 px-3 py-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div v-else class="space-y-1 font-mono text-xs">
              <div v-for="e in events" :key="e.id" class="flex items-baseline gap-2 rounded px-2 py-1 hover:bg-muted/50">
